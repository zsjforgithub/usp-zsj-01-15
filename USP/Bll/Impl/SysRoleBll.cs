...
namespace USP.Bll.Impl
{
    public class SysRoleBll : ISysRoleBll
    {
        ...
        public DataGrid<SysRole> getSysRolePageByCorp(long corp, int page, int pagesize)
        {
            var result = new DataGrid<SysRole>();
            long cnt;
            var roles = sysRoleDal.getSysRolePageByCorp(corp, page, pagesize, out cnt);
            result.rows = roles;
            result.total = cnt;
            return result;
        }
